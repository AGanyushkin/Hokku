...
const KAKKA_DISCOVERY_MESSAGE_PREFIX = 'KAKKA_DISCOVERY';
const KAKKA_DISCOVERY_PICKUP_ME = 'PICKUP_ME';
...
function discoveryRequestPICKUP_ME(
    appId: ApplicationIdType,
    serviceId: ServiceIdType,
    instanceId: InstanceIdType,
    pipePort: number
) {
    const cmd = `${KAKKA_DISCOVERY_MESSAGE_PREFIX}:${KAKKA_DISCOVERY_PICKUP_ME}`;
...
function isDiscoveryRequest(msg: string): boolean {
    ...
    return token[0] === KAKKA_DISCOVERY_MESSAGE_PREFIX;
}

function isDiscoveryRequestPICKUP_ME(msg: string): boolean {
    const token = `${msg}`.split(':');

    return token[1] === KAKKA_DISCOVERY_PICKUP_ME;
}
...
function handleIncomingDiscoveryEventFactory(discoverySubject: Rx.Subject) {
    return function handleIncomingDiscoveryEvent(msg: string, rInfo: RinfoType) {
        try {
            ...
            if (isDiscoveryRequestPICKUP_ME(msg)) {
...
export function discoveryBeacon(hokku: HokkuType) {
    ...
    const discoveryMessage = discoveryRequestPICKUP_ME(
        hokku.opts.app,
        hokku.opts.service,
